...
# 🧩 Anatomy of a ExtenRe patch

Learn the API to create patches using ExtenRe Patcher.
...
```kt
package com.extenre.patches.ads
...
Multiple types are already built into ExtenRe Patcher and are supported by any application that uses ExtenRe Patcher.
...
ExtenRe Patcher merges the classes from the extension into `context.classes` before executing the patch.
When the patch is executed, it can reference the classes and methods from the extension.

> [!NOTE]
>
> The [ExtenRe Patches template](https://github.com/LuisCupul04/ExtenRe-patches-template) repository
...
The next page explains the concept of fingerprinting in ExtenRe Patcher.
