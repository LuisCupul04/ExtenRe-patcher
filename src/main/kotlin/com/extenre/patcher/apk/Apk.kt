...
sealed class Apk private constructor(internal val module: ApkModule) {
    ...
    inner class ResourceContainer(internal val tableBlock: TableBlock?) {
        ...
        fun openFile(path: String) = ResourceFile(
            createHandle(path), archive, this
        )
    ...
    internal inner class BytecodeData {
        ...
        internal fun writeDexFiles() {
            // Apply proxies to the classes (this modifies the classes in memory)
            val classes = classes.also(ProxyBackedClassList::applyProxies)
            val opcodes = opcodes

            // Create a temporary directory for the modified DEX files
            val tempDir = Files.createTempDirectory("extenre_dex").toFile()
            try {
                ...
                // Write the modified DEX files to the temporary directory
                MultiDexIO.writeDexFile(
                    true,                     // verbose
                    -1,                       // number of threads (auto)
                    tempDir,                  // output directory
                    Patcher.dexFileNamer,      // dex file namer
                    newDexFile,                // the DexFile with modified classes
                    DexIO.DEFAULT_MAX_DEX_POOL_SIZE,
                    null                       // existing dex names (optional)
                )

                // Add each generated .dex file to the APK
